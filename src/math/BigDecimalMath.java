...
public class BigDecimalMath {
   ...
   * @since 2009-08-16
   */
  public static BigDecimal cbrt(final BigDecimal x) {
    if (x.compareTo(BigDecimal.ZERO) < 0)
   ...
   * @since 2009-06-25
   */
  public static BigDecimal hypot(final BigDecimal x, final BigDecimal y) {
                /* compute x^2+y^2
                */
   ...
   * @since 2009-08-05
   */
  public static BigDecimal hypot(final int n, final BigDecimal x) {
  ...
  static private int TAYLOR_N_TERM = 8;
  ...
  public static BigDecimal exp(BigDecimal x) {
    ...
    if (x.compareTo(BigDecimal.ZERO) < 0) {
      ...
      return scalePrec(BigDecimal.ONE, -(int) (Math.log10(x.ulp().doubleValue())));
                        ...
                        * value where TAYLOR_N_TERM terms will do. If x<1, the n-th term is of the order
                        ...
                        * We want x^TAYLOR_N_TERM/TAYLOR_N_TERM! < x.ulp, which is guaranteed if
                        * x^TAYLOR_N_TERM < TAYLOR_N_TERM*(TAYLOR_N_TERM-1)*...*x.ulp.
                        */
      final double xDbl = x.doubleValue();
      final double xUlpDbl = x.ulp().doubleValue();
      if (Math.pow(xDbl, TAYLOR_N_TERM) < TAYLOR_N_TERM * (TAYLOR_N_TERM - 1.0) * (TAYLOR_N_TERM - 2.0) * xUlpDbl) {
                                /* Add TAYLOR_N_TERM terms of the Taylor expansion (Euler's sum formula)
                                */
        BigDecimal resul = BigDecimal.ONE;
        ...
        BigInteger ifac = BigInteger.ONE;
                                ...
                                /* TAYLOR_N_TERM terms to be added means we move x.ulp() to the right
                                * for each power of 10 in TAYLOR_N_TERM, so the addition won't add noise beyond
                                * what's already in x.
                                */
        MathContext mcTay = new MathContext(err2prec(1., xUlpDbl / TAYLOR_N_TERM));
        for (int i = 1; i <= TAYLOR_N_TERM; i++) {
        ...
        return resul.round(mc);
                                ...
                                * to loss of accuracy.
                                */
        int exSc = (int) (1.0 - Math.log10(TAYLOR_N_TERM * (TAYLOR_N_TERM - 1.0) * (TAYLOR_N_TERM - 2.0) * xUlpDbl
          / Math.pow(xDbl, TAYLOR_N_TERM)) / (TAYLOR_N_TERM - 1.0));
   ...
   * @since 2009-08-08
   */
  public static BigDecimal log(int n, final MathContext mc) {
                /* the value is undefined if x is negative.
                */
    if (n <= 0)
   ...
   * @since 2009-08-09
   */
  public static BigDecimal log(final Rational r, final MathContext mc) {
                /* the value is undefined if x is negative.
                */
  ...
  public static BigDecimal sin(final BigDecimal x) {
...

    {
      ...
      if (res.compareTo(p) > 0) {
        ...
        */
        if (res.multiply(BigDecimal.valueOf(4L)).compareTo(p) > 0) {
          /* x>pi/4: sin(x) = cos(pi/2-x) */
          return cos(subtractRound(p.divide(BigDecimal.valueOf(2L)), res));
        }

        {
          ...
          /* x^i */
          BigDecimal xPowerI = res;

          /* 2i+1 factorial */
          BigInteger iFactor = BigInteger.ONE;
          ...
          */
          ...
          for (int i = 1; ; i++) {
            ...
            iFactor = iFactor.multiply(BigInteger.valueOf(2 * i));
            iFactor = iFactor.multiply(BigInteger.valueOf(2 * i + 1));
            xPowerI = xPowerI.multiply(res).multiply(res).negate();
            BigDecimal corr = xPowerI.divide(new BigDecimal(iFactor), mcTay);
            result = result.add(corr);
...

    {
       ...
       */
        if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
          ...
          return sin(subtractRound(p.divide(BigDecimal.valueOf(2L)), res));
        }

        {
          ...
          /* x^i */
          BigDecimal xPowerI = BigDecimal.ONE;
          ...
          */
          ...
          mc = new MathContext(err2prec(result.doubleValue(), xUlpDbl));
          return result.round(mc);
        }
      }
    }
  ...
  public static BigDecimal tan(final BigDecimal x) {
    ...
    if (x.compareTo(BigDecimal.ZERO) < 0) return tan(x.negate()).negate();

    {
...

      {
        ...
        for (int i = 2; ; i++) {
          ...
          xPowerI = multiplyRound(xPowerI, xHighPrSq);
          BigDecimal c = multiplyRound(xPowerI, f);
          result = result.add(c);
   ...
   * @since 2009-07-31
   */
  public static BigDecimal cot(final BigDecimal x) {
    if (x.compareTo(BigDecimal.ZERO) == 0) throw new ArithmeticException("Cannot take cot of zero " + x.toString());

    if (x.compareTo(BigDecimal.ZERO) < 0) return cot(x.negate()).negate();

    {
      ...
      BigDecimal result = BigDecimal.ONE.divide(xHighPr, mc);
      ...
      mc = new MathContext(err2prec(result.doubleValue(), eps));
      return result.round(mc);
    }
  } /* BigDecimalMath.cot */
  ...
  public static BigDecimal asin(final BigDecimal x) {
    ...
    if (x.compareTo(BigDecimal.ZERO) < 0) return asin(x.negate()).negate();
    ...
    if (x.doubleValue() > 0.7) {
      ...
      /* i factorial */
      BigInteger iFactorN = BigInteger.ONE;
      BigInteger iFactorD = BigInteger.ONE;

      for (int i = 1; ; i++) {
        iFactorN = iFactorN.multiply(BigInteger.valueOf(2 * i - 1));
        iFactorD = iFactorD.multiply(BigInteger.valueOf(i));
        if (i == 1) {
          xPowerI = xHighPrV;
        } else {
          xPowerI = multiplyRound(xPowerI, xHighPrV);
        }
        BigDecimal c = divideRound(multiplyRound(xPowerI, iFactorN), iFactorD.multiply(BigInteger.valueOf(2 * i + 1)));
        result = result.add(c);
      ...
      /* sqrt(2*z)*(1+...) */
      xPowerI = sqrt(xHighPr.multiply(new BigDecimal(2)));
      result = multiplyRound(xPowerI, result);

      MathContext mc = new MathContext(result.precision());
      BigDecimal piHalf = pi(mc).divide(BigDecimal.valueOf(2));

      mc = new MathContext(err2prec(result.doubleValue(), eps));
      return piHalf.subtract(result, mc);
    }
    ...
    {/* absolute error in the result is err(x)/sqrt(1-x^2) to lowest order */
      ...
      for (int i = 1; ; i++) {
        iFactorN = iFactorN.multiply(BigInteger.valueOf(2 * i - 1));
        iFactorD = iFactorD.multiply(BigInteger.valueOf(2 * i));
        xPowerI = multiplyRound(xPowerI, xHighPrSq);
        BigDecimal c = divideRound(multiplyRound(xPowerI, iFactorN), iFactorD.multiply(BigInteger.valueOf(2 * i + 1)));
        result = result.add(c);
        if (Math.abs(c.doubleValue()) < 0.1 * eps) {
  ...
  public static BigDecimal acos(final BigDecimal x) {
    ...
    final BigDecimal xHighPr = scalePrec(x, 2);
    BigDecimal result = asin(xHighPr);
    double eps = result.ulp().doubleValue() / 2.;

    MathContext mc = new MathContext(err2prec(3.14159, eps));
    BigDecimal piHalf = pi(mc).divide(BigDecimal.valueOf(2));
    result = piHalf.subtract(result);
  ...
  public static BigDecimal atan(final BigDecimal x) {
    if (x.compareTo(BigDecimal.ZERO) < 0) return atan(x.negate()).negate();
    ...
    if (x.doubleValue() > 0.7 && x.doubleValue() < 3.0) {
      ...
      BigDecimal result = multiplyRound(atan(newX), 2);
    ...
    if (x.doubleValue() < 0.71) {
      ...
      for (int i = 1; ; i++) {
        xPowerI = multiplyRound(xPowerI, xHighPrSq);
        BigDecimal c = divideRound(xPowerI, 2 * i + 1);
        result = result.add(c);
...

    {
      ...
      BigDecimal result = onePi.divide(new BigDecimal(2));
  ...
  public static BigDecimal cosh(final BigDecimal x) {
    if (x.compareTo(BigDecimal.ZERO) < 0) return cosh(x.negate());
...

    {
...

      {
        ...
        * the absolute value will give a safe relative error estimate for the individual terms
        */
        MathContext mcTay = new MathContext(err2prec(1., xUlpDbl / k));
        for (int i = 1; ; i++) {
          /* TBD: at which precision will 2*i-1 or 2*i overflow? */
          iFactor = iFactor.multiply(BigInteger.valueOf(2 * i - 1));
          iFactor = iFactor.multiply(BigInteger.valueOf(2 * i));
          xPowerI = xPowerI.multiply(xHighPr).multiply(xHighPr);
          BigDecimal corr = xPowerI.divide(new BigDecimal(iFactor), mcTay);
          result = result.add(corr);
        ...
        /* The error in the result is governed by the error in x itself. */
        MathContext mc = new MathContext(err2prec(result.doubleValue(), xUlpDbl));
        return result.round(mc);
      }
    }
  } /* BigDecimalMath.cosh */
  ...
  public static BigDecimal sinh(final BigDecimal x) {
...

    {
      if (x.doubleValue() > 2.4) {
        ...
        BigDecimal result = sinh(xHalf).multiply(cosh(xHalf)).multiply(two);
        /* The error in the result is set by the error in x itself.
...

      {
        ...
        */
        ...
        for (int i = 1; ; i++) {
          /* TBD: at which precision will 2*i or 2*i+1 overflow? */
          iFactor = iFactor.multiply(new BigInteger("" + (2 * i)));
          iFactor = iFactor.multiply(new BigInteger("" + (2 * i + 1)));
          xPowerI = xPowerI.multiply(xHighPr).multiply(xHighPr);
          BigDecimal corr = xPowerI.divide(new BigDecimal(iFactor), mcTay);
          result = result.add(corr);
          if (corr.abs().doubleValue() < 0.5 * xUlpDbl) {
   ...
   * @since 2009-08-20
   */
  public static BigDecimal asinh(final BigDecimal x) {
    if (x.compareTo(BigDecimal.ZERO) == 0) return BigDecimal.ZERO;
   ...
   * @since 2009-08-20
   */
  public static BigDecimal acosh(final BigDecimal x) {
   ...
   * @since 2009-08-06
   */
  public static BigDecimal Gamma(final BigDecimal x) {
   ...
   * @since 2010-05-26
   */
  public static BigDecimal Gamma(final Rational q, final MathContext mc) {
    if (q.isBigInteger()) {
   ...
   * @since 2009-08-19
   */
  public static BigDecimal pochhammer(final BigDecimal x, final int n) {
                /* reduce to interval near 1.0 with the functional relation, Abramowitz-Stegun 6.1.33
                */
    if (n < 0)
   ...
   * @since 2009-08-05
   */
  public static BigDecimal zeta(final int n, final MathContext mc) {
    if (n <= 0)
   ...
   * @since 2009-08-20
   */
  public static double zeta1(final int n) {
   ...
   * @since 2009-08-26
   */
  public static double psi(final double x) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal addRound(final BigDecimal x, final BigDecimal y) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex addRound(final BigComplex x, final BigDecimal y) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex addRound(final BigComplex x, final BigComplex y) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal subtractRound(final BigDecimal x, final BigDecimal y) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex subtractRound(final BigComplex x, final BigComplex y) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal multiplyRound(final BigDecimal x, final BigDecimal y) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex multiplyRound(final BigComplex x, final BigDecimal y) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex multiplyRound(final BigComplex x, final BigComplex y) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal multiplyRound(final BigDecimal x, final Rational f) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal multiplyRound(final BigDecimal x, final int n) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal multiplyRound(final BigDecimal x, final BigInteger n) {
    BigDecimal resul = x.multiply(new BigDecimal(n));
                /* The estimation of the absolute error in the result is |n*err(x)|
                */
   ...
   * @since 2009-07-30
   */
  public static BigDecimal divideRound(final BigDecimal x, final BigDecimal y) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex invertRound(final BigComplex z) {
   ...
   * @since 2010-07-19
   */
  public static BigComplex divideRound(final BigComplex x, final BigComplex y) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal divideRound(final BigDecimal x, final int n) {
   ...
   * @since 2009-07-30
   */
  public static BigDecimal divideRound(final BigDecimal x, final BigInteger n) {
                /* The estimation of the relative error in the result is |err(x)/x| 
                */
    MathContext mc = new MathContext(x.precision());
    return x.divide(new BigDecimal(n), mc);
   ...
   * @since 2009-08-05
   */
  public static BigDecimal divideRound(final BigInteger n, final BigDecimal x) {
                /* The estimation of the relative error in the result is |err(x)/x| 
                */
    MathContext mc = new MathContext(x.precision());
   ...
   * @since 2012-03-01
   */
  public static BigComplex divideRound(final BigInteger n, final BigComplex x) {
   ...
   * @since 2009-08-05
   */
  public static BigDecimal divideRound(final int n, final BigDecimal x) {
                /* The estimation of the relative error in the result is |err(x)/x| 
                */
    MathContext mc = new MathContext(x.precision());
    return new BigDecimal(n).divide(x, mc);
   ...
   * @author Richard J. Mathar
   */
  public static BigDecimal scalePrec(final BigDecimal x, int d) {
   ...
   * @author Richard J. Mathar
   */
  public static BigComplex scalePrec(final BigComplex x, int d) {
   ...
   * @since 2009-08-09
   */
  public static double prec2err(final double x, final int prec) {
